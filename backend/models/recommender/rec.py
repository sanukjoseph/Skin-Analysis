...
csv_path_final = os.path.join(script_dir, 'final.csv')
csv_path_makeup = os.path.join(script_dir, 'makeup_final.csv')

try:
    df2 = pd.read_csv(csv_path_final)
    makeup = pd.read_csv(csv_path_makeup)
except FileNotFoundError as e:
    print(f"!!! Error loading CSV file: {e}")
    # Handle error appropriately - maybe raise it again or exit
    raise e # Re-raise the exception to stop execution if files are critical
